...
export function AIChatBox({
  ...
  const displayMessages = messages.filter(
    (message): message is Exclude<Message, { role: "system" }> =>
      message.role !== "system"
  );
  ...
  return (
    <section
      className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[28px] border border-white/8 bg-[#0c1526]/88 shadow-[0_24px_80px_rgba(0,0,0,0.25)] backdrop-blur-xl"
    ...
    >
        ...
        {displayMessages.length === 0 ? (
          <div className="flex h-full min-h-[360px] flex-col items-center justify-center text-center animate-fade-up">
            ...
            <div className="mt-7 grid max-w-2xl grid-cols-1 gap-3 sm:grid-cols-3">
              {["Analyze an idea", "Improve a draft", "Plan a project"].map(
                (item, index) => (
                  <div
                    key={item}
                    className="rounded-2xl border border-white/7 bg-white/[0.025] px-4 py-3 text-left text-sm text-slate-300 animate-fade-up"
          ...
          <div className="mx-auto flex max-w-4xl flex-col gap-7">
            {displayMessages.map((message, index) => (
              ...
              >
                ...
                <div
                  className={cn(
                    "max-w-[85%] rounded-2xl px-4 py-3.5 text-sm leading-6 shadow-sm sm:max-w-[78%]",
                    message.role === "user"
                      ? "rounded-tr-md bg-gradient-to-br from-cyan-300 to-sky-400 text-slate-950"
                      : "rounded-tl-md border border-white/7 bg-white/[0.045] text-slate-100"
                  )}
                >
                  {message.role === "assistant" ? (
            ...
            {isLoading && (
              <div className="flex items-center gap-3 animate-fade-up">
                <div className="grid size-8 place-items-center rounded-xl border border-cyan-200/15 bg-cyan-300/8">
                  <Sparkles className="size-4 text-cyan-200" />
                </div>
      ...
      <div className="border-t border-white/7 bg-[#0a1221]/80 p-3 sm:p-5">
        <form
          onSubmit={submit}
          className={cn(
            "mx-auto flex max-w-4xl items-end gap-3 rounded-2xl border bg-white/[0.035] p-2.5 transition-all duration-200",
            hasFocused
              ? "border-cyan-200/35 shadow-[0_0_0_4px_rgba(34,211,238,0.06)]"
              : "border-white/9"
          )}
        >
          <Textarea
          ...
          <Button
            ...
            size="icon"
            className="size-10 shrink-0 rounded-xl bg-cyan-300 text-slate-950 shadow-[0_8px_28px_rgba(34,211,238,0.25)] transition-transform duration-150 hover:bg-cyan-200 active:scale-95 disabled:bg-slate-700 disabled:text-slate-400"
          >
            {isLoading ? (
        ...
        {suggestedPrompts?.length ? (
          <div className="mx-auto mt-2 flex max-w-4xl flex-wrap gap-2 px-2">
            {suggestedPrompts.map(prompt => (
              <button
                key={prompt}
                onClick={() => updateInput(prompt)}
                className="rounded-lg border border-white/8 px-2.5 py-1 text-xs text-slate-400 hover:border-cyan-200/25 hover:text-cyan-100"
              >
                {prompt}
              </button>
            ))}
        ...
        <p className="mx-auto mt-2 max-w-4xl px-2 text-center text-[11px] text-slate-500">
          Server-side AI gateway · Markdown ready · Your API keys never reach
          the browser
        </p>
      </div>
    </section>
