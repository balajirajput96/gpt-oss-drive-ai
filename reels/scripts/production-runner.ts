import { beginNextReel, loadProgram, markDriveVerified, recordFailure, reverifyDriveArtifact, selectNextReel } from "../lib/reelWorkflow";
...
if (command === "inspect") {
  ...
  if (!reelId || !videoFileId || !metadataFileId || !sha256 || !canonicalFolderId) {
    throw new Error("Usage: reverify <reelId> <videoFileId> <metadataFileId> <sha256> <canonicalFolderId>");
  }
  console.log(JSON.stringify(reverifyDriveArtifact(rootDir, reelId, { videoFileId, metadataFileId, sha256, canonicalFolderId }), null, 2));
} else {
