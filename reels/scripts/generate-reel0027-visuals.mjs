...
const outputs = scenes.map((art, index) => { const scene = String(index + 1).padStart(2, "0"); const svg = `${sourceDir}/reel0027_scene${scene}.svg`; const png = `${outputDir}/reel0027_scene${scene}.png`; writeFileSync(svg, wrap(art, scene)); const result = spawnSync("ffmpeg", ["-y", "-i", svg, "-frames:v", "1", png]); if (result.status !== 0) throw new Error(`Scene ${scene} failed`); return { scene, svg, png }; });
writeFileSync(`${assetDir}/REEL-0027_VISUAL_PROVENANCE.json`, JSON.stringify({ reelId: "0027", visualRoute: "original_deterministic_svg_motion_graphics", imageGenerationUsed: false, quotaOrAccessControlBypass: false, reusedLegacyAsset: false, embeddedText: false, realPersonDepicted: false, scenes: outputs }, null, 2));
