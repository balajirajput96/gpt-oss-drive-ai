...
const provenance = { reelId: "0011", visualRoute: "original_deterministic_svg_motion_graphics", embeddedText: false, imageGenerationUsed: false, quotaOrAccessControlBypass: false, scenes: [] };
for (const [index, svg] of scenes.entries()) {
  ...
  const conversion = spawnSync("ffmpeg", ["-y", "-i", svgPath, "-vf", "format=rgba", "-frames:v", "1", pngPath], { encoding: "utf8" });
  if (conversion.status !== 0) throw new Error(conversion.stderr || `Could not rasterize scene ${scene}`);
  provenance.scenes.push({ scene, source: "deterministic_svg", sourcePath: svgPath, outputPath: pngPath, containsText: false });
}
writeFileSync("/home/ubuntu/reel-production-assets/REEL-0011/visual_provenance.json", `${JSON.stringify(provenance, null, 2)}\n`);
