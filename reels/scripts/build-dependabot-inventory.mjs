...
const detailSource = "/home/ubuntu/console_outputs/exec_result_2026-08-23_02-50-42_866.txt";
const detailExtraction = JSON.parse(JSON.parse(readFileSync(detailSource, "utf8")));
...
const alerts = detailExtraction.records
  .map(alert => ({
    ...
    evidence: "Authenticated GitHub Dependabot UI; post-security-hardening refresh observed 42 open and 99 closed alerts.",
  }))
  .sort((a, b) => a.alertId - b.alertId);

if (alerts.length !== 42 || new Set(alerts.map(alert => alert.alertId)).size !== 42) {
  throw new Error(`Expected 42 unique open alerts; found ${alerts.length} rows and ${new Set(alerts.map(alert => alert.alertId)).size} unique IDs.`);
}
...
const inventory = {
  ...
  productionAuditStatus: "pnpm audit --prod reports zero known vulnerabilities after remediation.",
  alerts,
};
...
const markdown = [
  ...
  "|---:|---|---|---|---|---|---|---|",
  ...alerts.map(alert => `| #${alert.alertId} | ${alert.package} | ${alert.affectedVersionOrRange} | ${alert.patchedVersion} | ${alert.severity} | ${alert.scope} | ${alert.relationship} | ${alert.disposition} |`),
  "",
  "## Disposition rule",
  "",
...
writeFileSync(resolve(root, "SECURITY_DEPENDABOT_INVENTORY_20260823.json"), `${JSON.stringify(inventory, null, 2)}\n`);
writeFileSync(resolve(root, "SECURITY_DEPENDABOT_INVENTORY_20260823.md"), `${markdown}\n`);
console.log(JSON.stringify({ alertCount: alerts.length, packages: [...new Set(alerts.map(alert => alert.package))] }, null, 2));
