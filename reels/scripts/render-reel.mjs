...
const [
  ...
  captionsPath,
  outputPath,
  voiceFilename = "REEL-0002_voice_final.wav",
  imagePrefix = "reel0002_scene",
] = process.argv.slice(2);
if (!assetDir || !captionsPath || !outputPath) {
  throw new Error(
    "Usage: node render-reel.mjs <assetDir> <captions.srt> <output.mp4> [voiceFilename] [imagePrefix]"
  );
}

const voicePath = resolve(assetDir, voiceFilename);
const imagePaths = [1, 2, 3, 4, 5, 6].map(number =>
  resolve("/home/ubuntu/webdev-static-assets", `${imagePrefix}0${number}.png`)
);
for (const requiredPath of [voicePath, captionsPath, ...imagePaths]) {
  if (!existsSync(requiredPath))
    throw new Error(`Missing render input: ${requiredPath}`);
}
...
if (probe.status !== 0)
  throw new Error(probe.stderr || "Could not determine narration duration.");
...
const filterParts = imagePaths.map(
  (_, index) =>
    `[${index}:v]scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,zoompan=z='min(zoom+0.00045,1.055)':d=${frameCount}:s=720x1280:fps=30,setsar=1[v${index}]`
);
filterParts.push(
  `${imagePaths.map((_, index) => `[v${index}]`).join("")}concat=n=${imagePaths.length}:v=1:a=0[base]`
);
const subtitlePath = resolve(captionsPath)
  .replaceAll("\\", "\\\\")
  .replaceAll(":", "\\:")
  .replaceAll("'", "\\'");
...
if (render.status !== 0)
  throw new Error(render.stderr || "FFmpeg render failed.");
console.log(
  JSON.stringify(
    {
      outputPath,
      narrationSeconds: duration,
      sceneSeconds: Number(sceneDuration),
      frameCount,
    },
    null,
    2
